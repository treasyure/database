...
def t_find_by_template():
    ...
    r_dbt = RDBDataTable("Batting", connect_info, key_columns=key_cols)

    res = r_dbt.find_by_template(template=tmp, field_list=fields)
...
def t_find_by_pk():
    ...
    r_dbt = RDBDataTable("batting", connect_info, key_columns=key_cols)
    res = r_dbt.find_by_primary_key(key_vals, fields)
    print("Query result= \n", json.dumps(res, indent=2))
...
def t_insert():
    ...
    rdb_tbl = RDBDataTable("orderdetails", connect_info, key_columns=['productCode'])
    tmp = {'orderNumber': "10025"}
    res_bef = rdb_tbl.find_by_template(template=tmp, field_list=fields)
    print("Query result= \n", json.dumps(res_bef, indent=2))

    res = rdb_tbl.insert(new_item)
    res_aft = rdb_tbl.find_by_template(template=tmp, field_list=fields)
...
def t_delete_by_template():
    ...
    rdb_tbl = RDBDataTable("orderdetails", connect_info,
...
def t_delete_by_key():
    ...
    fields = ['orderNumber', 'productCode']
    key_vals = ['10100', '2']
    r = rdb_tbl.delete_by_key(key_fields=key_vals)
...
def t_update_by_template():
    ...
    r1 = rdb_tbl.find_by_template({"orderNumber": "10025"}, field_list=fields)

    print("Details for order '10025' = \n", json.dumps(r1, indent=2))

    print("\nUpdate productCode 'S18_1749':")
    r2 = rdb_tbl.update_by_template({"orderNumber": "10100", "productCode": 'S18_4409'},{"orderNumber": "10025", "productCode": 'S18_3171'})

    print("update returned ", r2, "\n")

    r3 = rdb_tbl.find_by_template({"orderNumber": "10025"}, field_list=fields)
...
def t_update_by_key():
    ...
    r = rdb_tbl.update_by_key(key_vals, {"orderNumber": "10101", "productCode": 'S18_3171'})
